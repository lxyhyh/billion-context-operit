...
node -e "
const src = require('fs').readFileSync('packages/bili_manager.js','utf8');
...
rm -rf dist
mkdir -p dist
zip -rq "dist/${PKG_ID}-v${VERSION}.toolpkg" main.js manifest.json packages ui -x '*.git*'
unzip -l "dist/${PKG_ID}-v${VERSION}.toolpkg" | grep -E 'manifest.json|main.js|packages/bili_manager|ui/bili_console'
